"""
Strategy: BTC Trend 4H
Generated by Atlas analytics discovery pipeline.
Optimized parameters: fast_period=11, slow_period=20
"""
import numpy as np
import pandas as pd
...
def generate_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    fast_ma = pd.Series(close).rolling(window=11).mean().values
    slow_ma = pd.Series(close).rolling(window=20).mean().values
