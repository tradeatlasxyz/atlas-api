"""
Strategy: ETH Trend 1D
Generated by Atlas analytics discovery pipeline.
Optimized parameters: fast_period=29, slow_period=31
"""
import numpy as np
import pandas as pd
...
def generate_signals(df: pd.DataFrame) -> np.ndarray:
    ...
    fast_ma = pd.Series(close).rolling(window=29).mean().values
    slow_ma = pd.Series(close).rolling(window=31).mean().values
